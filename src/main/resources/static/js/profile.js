...
/**
 * Выполняет запрос для проверки доступа к админской панели.
 * При успешном ответе перенаправляет на страницу админской панели, иначе показывает сообщение об ошибке.
 */
async function goToAdminPanel() {
    try {
        ...
        if (response.ok) {
            window.location.href = '/admin'; // Перенаправление на админскую панель
        } else if (response.status === 400) {
            showErrorMessage('Недостаточно прав для доступа к админской панели. Требуется админская роль.');
        } else {
            showErrorMessage(`Ошибка при попытке открыть админскую панель.`);
        }
    } catch (error) {
        console.error('Ошибка сети или сервера:', error);
        showErrorMessage('Ошибка сети или сервера.');
    }
}
